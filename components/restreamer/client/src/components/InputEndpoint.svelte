<script lang="js">
    ...
    import {Info, SetEndpointLabel} from '../../api/client.graphql';
    ...
    async function editLabel(startEdit) {
        if (startEdit) {
            ...
            const variables = { restream_id: restream_id, input_id: input.id, endpoint_id: endpoint.id, label: label_input.value };
            try {
...
<template>
    <div class="endpoint">
        ...
        <div class="endpoint-label">
            <span bind:this={label_component} class:hidden={editing_label}>{endpoint.label ? endpoint.label : ""}</span>
            {#if editing_label}
                <input bind:this={label_input} use:init_input on:focusout|preventDefault={() => {editLabel(false)}}/>
            {/if}
            <a class="edit-label" href="/" on:click|preventDefault={() => {editLabel(true)}}>
                <i class="far fa-edit" title="Edit label"></i>
            </a>
        </div>
    </div>
</template>
